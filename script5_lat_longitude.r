...
library(readr)
  ...
  df_municipios <- df_municipios %>% 
                   left_join(muni_lat_long, by = c("cod_municipio"="siafi_id")) %>% 
                   select(data, cod_municipio, codigo_ibge, codigo_uf, nome_municipio, nome, everything())


  
  write.csv(df_municipios, 
                   file=paste0("RECEITA_COOP_2023/dados_finais","/","df_municipios.csv"),
                      row.names = F) 
